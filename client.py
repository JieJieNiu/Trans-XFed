...
def train(args,model,global_model,mu):
    ...
    for epoch in tqdm(range(args.E)):
        ...
        for (seq, label) in Dtr:
            ...
            if args.M == "TransFed" or args.M =="FedProx":
                if args.loss=="NLL":
                    m=nn.LogSoftmax(dim=1)
                    loss = loss_function(m(y_pred), label.to(dtype=torch.long)) + (mu / 2) * proximal_term
                else:
                    loss = loss_function(y_pred, label.to(dtype=torch.long)) + (mu / 2) * proximal_term
            elif args.M =="FedAvg":
                if args.loss=="NLL":
                    m=nn.LogSoftmax(dim=1)
                    loss = loss_function(m(y_pred), label.to(dtype=torch.long))
                else:
                    loss = loss_function(y_pred, label.to(dtype=torch.long))
        ...
        print('model name {} epoch {:03d} mu{} train_loss {:.8f} val_loss {:.8f},Recall:{}, F1:{},Precision:{}'.format(model.name, epoch, mu,train_loss, val_loss,recall,F1,percision))
